use crate::{callsite::CallsiteIdentity, context::MezmoContext, set_pipeline_state_variable};
...
static PIPELINE_STATE_VARIABLE_CHANGE_ACTION_LOGGER: OnceLock<PipelineStateVariableChangeActionLogger> = OnceLock::new();
...
struct PipelineStateVariableChangeActionLogger {
    ...
    action_rate_limit_cache: moka::sync::Cache<PipelineStateVariableChangeIdentifier, Arc<Mutex<State>>>,
    rate_limit: u64,
}
...
impl PipelineStateVariableChangeActionLogger {
    fn new(rate_limit: u64) -> Self {
        PipelineStateVariableChangeActionLogger {
            ...
            action_rate_limit_cache: moka::sync::Cache::new(PIPELINE_STATE_VARIABLE_CHANGE_ACTION_CACHE_RATE_LIMIT_MAX_CAPACITY),
            rate_limit,
        }
    }
}
...
pub fn get_pipeline_state_variable_change_action_sender() -> &'static broadcast::Sender<LogEvent> {
    &PIPELINE_STATE_VARIABLE_CHANGE_ACTION_LOGGER
        .get_or_init(|| PipelineStateVariableChangeActionLogger::new(DEFAULT_RATE_LIMIT_UNINITIALIZED))
        .sender
}

pub fn try_send_pipeline_state_variable_change_action(log: LogEvent, id: PipelineStateVariableChangeIdentifier) {
    if let Some(pipeline_state_variable_change_action_logger) = PIPELINE_STATE_VARIABLE_CHANGE_ACTION_LOGGER.get() {
        let entry = pipeline_state_variable_change_action_logger.action_rate_limit_cache.entry(id).or_insert_with(|| {
        ...
        if let Ok(state) = Arc::<Mutex<State>>::clone(entry.value()).lock().as_mut() {
            ...
            if state.should_limit() {
                match previous_count {
                    0 => match pipeline_state_variable_change_action_logger.sender.send(log) {
                        Ok(_) => {}
                        Err(_) => {
                ...
                match pipeline_state_variable_change_action_logger.sender.send(log) {
                    Ok(_) => {}
                    Err(_) => {
                        debug!("failed to send pipeline event; likely no source consuming data")
                    }
                }
...
/// Defines a set of methods intended to enfore the share of pipeline events
pub trait PipelineStateVariableChangeActionLog {
    fn set_pipeline_state_variable(
        &self,
        name: String,
        value: String,
        identity: CallsiteIdentity,
...
impl PipelineStateVariableChangeActionLog for MezmoContext {
    fn set_pipeline_state_variable(
        &self,
        name: String,
        value: String,
        ...
        event.insert("meta.mezmo.pipeline_event_action", "set_pipeline_state_variable");
        ...
        event.insert("message.name", name);
        event.insert("message.value", value);
...
mod tests {
    ...
    async fn test_pipeline_event_set_pipeline_state_variable() {
        ...
        _ = internal_set_pipeline_state_variable(
            &ctx,
            TEST_VRL_POSITION,
        ...
        for ((exp_name, exp_value), actual) in expected_values.into_iter().zip(res.iter()) {
            let event_type = actual
                .get(".meta.mezmo.event_type")
                .expect("should contain a event_type type")
                .as_str()
            ...
            let name = actual
                .get(".message.name")
                .expect("should contain message value")
                .as_str()
    ...
    #[tokio::test]
    #[serial]
    async fn pipeline_state_variable_changetest_rate_limiting() {
        let id = "v1:js-script:internal_source:component_abc:pipeline_123:account_123".to_owned();
        let ctx = MezmoContext::try_from(id).ok();
        let mut log_stream = PipelineStateVariableChangeActionSubscription::subscribe().into_stream();

        for _ in 0..10 {
            set_pipeline_state_variable!(ctx, None, "var1", "value1");
        }
