...
use vector_lib::{
    ...
    usage_metrics::{
        ComponentUsageTracker, OutputUsageTracker, UsageMetrics, get_component_usage_tracker,
        get_transform_usage_tracker,
    },
};
    ...
    async fn build_sources(
            ...
            )
        {
            ...
            for output in source_outputs.into_iter() {
                ...
                let pump = run_source_output_pump(rx, fanout, source, source_type, usage_tracker);
    ...
    async fn build_transforms(
            ...
            .filter(|(key, _)| self.diff.transforms.contains_new(key))
        {
            ...
            let (transform_task, transform_outputs) =
                self.build_transform(transform, node, input_rx, usage_tracker);
    ...
    fn build_transform(
        ...
        match transform {
            ...
            Transform::Function(t) => {
                self.build_sync_transform(Box::new(t), node, input_rx, usage_tracker)
            }
            Transform::Synchronous(t) => {
                self.build_sync_transform(t, node, input_rx, usage_tracker)
            }
...
async fn run_source_output_pump(
    ...
    usage_tracker: Box<dyn ComponentUsageTracker>,
) -> TaskResult {
