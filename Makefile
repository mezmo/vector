...
test-behavior-%: ## Runs behavioral test for a given category
	$(call MAYBE_ENVIRONMENT_EXEC,cargo run --no-default-features --features transforms,vrl-functions-env,vrl-functions-system,vrl-functions-network -- test tests/behavior/$*/*)
...
test-vector-api: ## Runs vector API tests (top and tap)
	$(call MAYBE_ENVIRONMENT_EXEC,cargo nextest run --no-fail-fast --no-default-features --features vector-api-tests --test vector_api)

.PHONY: test-component-validation
test-component-validation: ## Runs component validation tests
	$(call MAYBE_ENVIRONMENT_EXEC,cargo nextest run --no-fail-fast --no-default-features --features component-validation-tests --status-level pass --test-threads 4 --lib components::validation::tests)
...
check-generated-docs: generate-docs ## Checks that the machine-generated component Cue docs are up-to-date.
	$(call MAYBE_ENVIRONMENT_EXEC,$(VDEV) check generated-docs)
...
generate-component-docs: ## Generate per-component Cue docs from the configuration schema.
	...
	$(call MAYBE_ENVIRONMENT_EXEC,./scripts/cue.sh fmt)

.PHONY: generate-vector-vrl-docs
generate-vector-vrl-docs: ## Generate VRL function documentation from Rust source.
	$(call MAYBE_ENVIRONMENT_EXEC,$(VDEV) build vector-vrl-docs --output docs/generated/ \
		$(if $(findstring true,$(CI)),>/dev/null,))
