...
generated: components: sources: mezmo_demo_logs: configuration: {
	count: {
		description: """
	...
	decoding: {
		...
		type: object: options: {
			...
			encoding: {
				description:   "Mezmo Deserializer variants"
				relevant_when: "codec = \"mezmo\""
				required:      true
				type: string: enum: {
					open_telemetry_logs:     "Open Telemetry Logs config"
					open_telemetry_metrics:  "Open Telemetry Metrics config"
					open_telemetry_traces:   "Open Telemetry Traces config"
					prometheus_remote_write: "Prometheus Remote Write config"
				}
			}
			gelf: {
	...
	devices: {
		description:   "number of devices that should generate records"
		relevant_when: "format = \"financial\""
		required:      false
		type: uint: default: 3
	}
	format: {
		...
		type: string: {
			...
			enum: {
				...
				env_sensor:      "Sensor data"
				financial:       "Financial Data"
				generic_metrics: "Generic metrics in the Mezmo metrics format"
				http_metrics:    "HTTP-based metrics in the Mezmo metrics format"
				...
				user_provided:   "User-provided logs to be infinitly looped sequentially"
			}
		}
	}
	framing: {
	...
	lines: {
		...
		relevant_when: "format = \"shuffle\" or format = \"user_provided\""
		required:      true
		type: array: items: type: string: {}
	}
	max_user_provided_lines: {
		description:   "The maximum number of user provided lines to loop over"
		relevant_when: "format = \"user_provided\""
		required:      false
		type: uint: default: 25
	}
	sequence: {
